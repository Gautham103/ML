...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores -= np.max(scores, axis=1, keepdims=True) # get the scores
        scores_exp = np.exp(scores) # Softmax numerater
        softmax_matrix = scores_exp / np.sum(scores_exp, axis=1, keepdims=True) # softmax matrix
